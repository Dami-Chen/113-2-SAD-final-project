...
export default function CreateOrder(){
  ...
  const [stopYear, setStopYear] = useState('');
 ...
 // 字的顏色
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setForm(initialFormState);
      const now = new Date();
      setDeliveryYear(String(now.getFullYear()));
      setDeliveryMonth(String(now.getMonth() + 1));
      setDeliveryDay(String(now.getDate()));
      setDeliveryHour("12");
      setStopYear(String(now.getFullYear()));
      setStopMonth(String(now.getMonth() + 1));
      setStopDay(String(now.getDate()));
      setClosingMethod('quantity');
      setAvatarUri(null);
      setCategory('');
      setHashtagInput('');
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 24 }}>
      ...
      <Text style={{ marginBottom: 4, color: '#333' }}>* 分送時間</Text>      
      <View style={styles.dateInputRow}>
        <TextInput
          placeholder="年 (YYYY)"
          style={styles.dateInput}
          ...
          value={deliveryYear}
          onChangeText={text => {
            setDeliveryYear(text);
            handleChange('delivery_time',`${text}-${deliveryMonth.padStart(2, '0')}-${deliveryDay.padStart(2, '0')}T${deliveryHour.padStart(2, '0')}:00:00Z`);
          }}
        />
        <TextInput
          placeholder="月 (MM)"
          ...
          value={deliveryMonth}
          onChangeText={text => {
            setDeliveryMonth(text);
            handleChange('delivery_time', `${deliveryYear}-${text}-${deliveryDay.padStart(2, '0')}T${deliveryHour.padStart(2, '0')}:00:00Z`);
          }}
        />
        <TextInput
          placeholder="日 (DD)"
          ...
          onChangeText={text => {
            setDeliveryDay(text);
            handleChange('delivery_time', `${deliveryYear}-${deliveryMonth.padStart(2, '0')}-${text.padStart(2, '0')}T${deliveryHour.padStart(2, '0')}:00:00Z`);
          }}
        />
        <TextInput
          placeholder="時 (HH)"
          ...
          value={deliveryHour}
          onChangeText={text => {
            setDeliveryHour(text);
            handleChange('delivery_time',`${deliveryYear}-${deliveryMonth.padStart(2, '0')}-${deliveryDay.padStart(2, '0')}T${text.padStart(2, '0')}:00:00Z`);          
          }}
        />
      </View>
      ...
      <View style={styles.radioRow}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            setClosingMethod('datetime');
            setShowDatePicker(true);
            setForm(prev => ({ ...prev, stop_at_date: form.stop_at_date, stop_at_num: form.quantity }));
          }}
        >
          <View style={styles.radioCircle}>
            {closingMethod === 'datetime' && <View style={styles.radioDot} />}
    ...
    {closingMethod === 'datetime' && (
      <View style={styles.dateInputRow}>
        <TextInput
          ...
          onChangeText={text => {
            setStopYear(text);
            handleChange('stop_at_date', `${text}-${stopMonth.padStart(2, '0')}-${stopDay.padStart(2, '0')}`);
          }}
        />
        <TextInput
          placeholder="月 (MM)"
          ...
          onChangeText={text => {
            setStopMonth(text);
            handleChange('stop_at_date', `${stopYear}-${text.padStart(2, '0')}-${stopDay.padStart(2, '0')}`);
          }}
        />
        <TextInput
          placeholder="日 (DD)"
          ...
          onChangeText={text => {
            setStopDay(text);
            handleChange('stop_at_date', `${stopYear}-${stopMonth.padStart(2, '0')}-${text.padStart(2, '0')}`);
          }}
        />
      </View>
      )}
